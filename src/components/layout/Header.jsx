...
function Header() {
  ...
  const submit = (e) => {
    ...
    if (location.pathname === "/allProduct") dispatch(getProduct(e, catagory));
    else if (location.pathname === "/allCatagories") dispatch(AllCatagory(e));
    else if (location.pathname === "/allVendors") dispatch(AllVendor(e));
  };
  const { catagory, error: catagoryError } = useSelector(
  ...
  useEffect(() => {
    if (localStorage.getItem("UserToken")) {
      if (catagoryError || VendorError) {
        if (catagoryError) toast.error(catagoryError);
  ...
  const createProductHandler = async () => {
    ...
    if (
      ...
      if (!createerror) toast.success("Product Created Successfully");
      setonclick(false);
  ...
  return (
    <Fragment>
      <div className="Header" style={loc ? { display: "none" } : {}}>
        <h2 className="Icon">HISAB RAKHO</h2>
        <TextField
          className="TextBoxHeader"
          label="Search input"
        ...
        <div>
          ...
          <Dialog open={onclick} onClose={closeCreateHandler}>
            ...
            <DialogContent>
              <DialogContentText>Enter Vendor's Name</DialogContentText>

              <Autocomplete
                id="VendorName"
                freeSolo
                value={VendorName}
                style={{ width: "28vmax", backgroundColor: "white" }}
                ...
                renderInput={(params) => (
                  <TextField
                    ...
                    InputProps={{
                      ...params.InputProps,
                      style: { color: "white" }, // Make the text white
                    }}
                    InputLabelProps={{
                      style: { color: "white" }, // Make the label white (if any)
                    }}
                  />
                )}
              />

              <DialogContentText>Enter Product Name</DialogContentText>
              <input
              ...
              />
              <DialogContentText>Enter price</DialogContentText>
              <input
                className="DialogInputFeild"
                autoFocus
                margin="dense"
              ...
              />
              <DialogContentText>Enter Catagory</DialogContentText>
              ...
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField
                    ...
                    {...params}
                    onInput={(e) => setProductCatagory(e.target.value)}
                  />
                )}
              />

              <DialogContentText>Enter Quantity</DialogContentText>
              <input
                className="DialogInputFeild"
                autoFocus
                margin="dense"
        ...
        <LogoutIcon
          className="Logout"
          style={!localStorage.getItem("UserToken") ? { display: "none" } : {}}
